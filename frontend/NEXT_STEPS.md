# 放置me — Gemini + Supabase 接続メモ

このドキュメントは、現在の実装をローカルから本番へ寄せるための手順です。Topic / clone-chat などの既定経路は Supabase Edge Functions、部屋住人・エンカウント会話は Next.js API routes です。

方針は固定です。

- ブラウザ公開変数には API キーを置かない
- `GEMINI_API_KEY` / `SUPABASE_SERVICE_ROLE_KEY` は server-only env として扱う
...
- 部屋住人・エンカウント会話: `frontend/src/app/api/room-chat/*`, `frontend/src/app/api/encounter/*`
...
- `NEXT_PUBLIC_*` だけがブラウザ公開される。
- `GEMINI_API_KEY` / `SUPABASE_SERVICE_ROLE_KEY` / `UPSTASH_*` は Next.js API routes 用の server-only env。
...
本番でも同じ考え方です。Edge Functions 用の `GEMINI_API_KEY` は Supabase 側 Secret に置きます。Next.js API routes 用の `GEMINI_API_KEY` / `UPSTASH_*` / `SUPABASE_SERVICE_ROLE_KEY` は Vercel の Environment Variables に置きます。
...
- Edge Functions 用の `GEMINI_API_KEY` が `backend/supabase/.env.local` に入っているか
- Next.js API routes 用の `GEMINI_API_KEY` / `UPSTASH_*` が `frontend/.env.local` または Vercel env に入っているか
