...
> **モック（正）**: [`reference/clone-os-mock.html`](./reference/clone-os-mock.html) — UI・3D・デザイントークンはここを唯一の参照源とする  
...
| モック | `project-docs/reference/clone-os-mock.html`（Three.js r128、単一HTML、Clone OS v2 UI） |
...
- [x] CSS 変数（`--neon-cyan`, `--bg-0` 等）と glass UI → `frontend/src/app/globals.css`
...
| 領域 | 状態 | 主なファイル |
|------|------|----------------|
| 3D + UI シェル | 完了（R3F 移植済み） | `frontend/src/components/world/WorldScene.tsx`, `Library.tsx`, `Avatar.tsx`, `Particles.tsx`, `RoomMarkers.tsx` |
...
DB（HO-101）と並行または直後に実施。`GEMINI_API_KEY` は公開環境変数にせず、Supabase Edge Functions Secret または Next.js API routes の server-only env として扱う。
...
| 17 | ✅ **HO-404** + ✅ **HO-405** | 王蕙鈺 / 阿部勝寿 | 毎日の質問 UI + 回答送信 | TopBar overlay から **HO-116** を呼ぶ |
...
| 順 | ID | 担当 | やること | 完了の目安 |
|----|-----|------|----------|------------|
| 33 | **HO-701** | 中村悠紀 / 柴沼勇太 | デザイン基準 doc（参照モックとの差分チェックリスト） | `project-docs/design-guide.md` |
...
| AI | **Google Gemini API**（`models/gemini-2.5-flash` 既定）— Edge Functions / Next API routes 経由でキーを秘匿 | `GEMINI_API_KEY` は server-only env。`NEXT_PUBLIC_*` にしない |
...
|----------------|------|------------|
| `/onboarding` | クローン作成 | — |
| `/` または `/world` | ホーム＝仮想世界ビュー | `project-docs/reference/clone-os-mock.html` 全体 |
...
- **実行場所**: Topic / clone-chat は Supabase **Edge Functions**（Deno）を本線。部屋住人・エンカウント会話は Next.js API routes。
...
- **Secret 方針**: `GEMINI_API_KEY` は server-only env のみ。Supabase Edge Functions Secret または Vercel Environment Variables に置き、`NEXT_PUBLIC_*` には置かない。
...
| **past_likes / like_reasons** | `clones` に past_likes 列を追加。reason は `clones.like_reasons JSONB` か Gemini 動的生成 | 阿部勝寿 |
...
| **agent-chat Edge Function** | ChatPanel から AI エージェントを呼ぶ経路。固定返信を Gemini 出力に差し替える | 阿部勝寿 |
| **agent-monologue（Should）** | 部屋アバターのモノローグを roster ごとに動的化 | 阿部勝寿 |
| **clone-chat 本番化** | 自分のクローン経由のチャット（HO-113 を完成） | 阿部勝寿 |
| **インフラ** | `GEMINI_API_KEY` のデプロイ、RLS（`friendships` / `agents` / `agent_dialogues`）、Realtime（Should） | 菅家孝太郎 / 阿部勝寿 |

FE 側のモック差し替えポイント：
- `lib/storage.ts` 認証ガードを `ensureUser` に統一
- `lib/store.ts` の `generateFriendId()` をログイン時取得に
- `lib/clone-engine.ts` を `LLMMockImpl` → `SupabaseEdgeFunctionImpl`
...
| Day | ゴール | デモで見せるもの |
|-----|--------|------------------|
| **Day 1** | モック移植 + クローン作成 + DB 新規 | 参照モック相当の 3D が React で動く。オンボーディング完了で Mira 表示 |
...
| HO-003 | Must | 参照モックのレイアウトを React シェルに移植（3D除く） | `AppShell`, 各パネル | 柴沼勇太 |
...
| HO-203 | Should | ミニマップ（全クローン位置・発話者パルス） | `WorldHudMenu` へ再実装 | 柴沼勇太 |
| HO-204 | Must | 左: クローンカード・バイタル・同期率表示 | DB `clones.vitals`, `sync_rate` | 柴沼勇太 |
| HO-205 | Must | 「いま」— 現在 activity を表示 | overlay / store + DB | 柴沼勇太 / 王蕙鈺 |
| HO-206 | Must | 今日のタイムライン一覧 | overlay / store + DB | 柴沼勇太 / 王蕙鈺 |
| HO-207 | Must | 操作・チャット導線 UI（送信は Phase E で接続） | `WorldHudMenu` / `ChatPanel` | 柴沼勇太 |
...
| HO-222 | Should | 手動時に最寄りロケーション名を HUD 更新 | `WorldHudMenu` 連動 | 柴沼勇太 |
...
| HO-302 | Must | 今日のTopic表示（タイトル・理由・関連ノート3） | overlay / store | 柴沼勇太 / 王蕙鈺 |
...
| HO-306 | Must | ページツリー ↔ `notes.parent_id` | overlay 側で再実装 | 柴沼勇太 |
...
| ID | 優先 | タスク | 成果物 | 担当 |
|----|------|--------|--------|------|
| HO-401 | Must | クローンチャット FE 完成 → **HO-113** 接続 | `ChatPanel` + invoke | 王蕙鈺 |
| HO-402 | Must | クローンチャット画面 + クイック質問チップ | `ChatPanel.tsx` | 柴沼勇太 |
| HO-403 | Must | 操作導線 → チャット or 指示パース（「西の書架へ」） | `WorldHudMenu` / `ChatPanel` | 柴沼勇太 / 阿部勝寿 |
| ✅ HO-404 | Must | 毎日の質問マスタ + 回答 UI（5問） | TopBar から開く Daily Questions overlay | 王蕙鈺 |
| ✅ HO-405 | Must | 毎日の質問 FE → **HO-116** 接続 | 回答送信後に同期率・Vitals が更新 | 王蕙鈺 / 阿部勝寿 |
...
└── src/
    ...
    │   ├── encounter/            # クローン遭遇イベント overlay / trigger
    │   ├── overlay/              # 全体 overlay 集約
    │   ├── chat/                 # ChatPanel
...
**アバター（Mira デフォルト）**: 紫×シアン、フード型、胸コア発光 — `PALETTES.mira` in `project-docs/reference/clone-os-mock.html`
...
| リスク | 対策 | カット時 |
|--------|------|----------|
| Three.js 移植が重い | Day1 は参照モックを iframe 埋め込みでも可 | 後から React 統合 |
...
- [x] HO-003（`AppShell` / `TopBar` / `WorldHudMenu` / `ChatPanel` / `Overlays` に統合済み）
- [x] HO-004（R3F: `components/world/WorldScene.tsx`, `Library.tsx`, `Avatar.tsx`, `Particles.tsx`, `RoomMarkers.tsx`）
...
- [x] HO-117（モック UI → Gemini/DB 結果に差替。`ChatPanel` / `Overlays` / store に統合）
- [x] HO-118（`backend/supabase/functions/parse-clone-command/`）

### Phase C
- [x] HO-201（自動巡回 + HUD 連動：`WorldScene.tsx` / `WorldHudMenu.tsx`）
- [x] HO-202（カメラ操作：`WorldScene.tsx` / `OrbitControls`）
- [ ] HO-203（旧 `MiniMap` は未使用のため削除。必要なら `WorldHudMenu` 側に再実装）
- [ ] HO-204（旧クローンカード・バイタルは未使用のため削除。プロフィール overlay 側で再整理）
- [x] HO-205（最新 activity は store / overlay 表示へ統合）
- [x] HO-206（当日 activity は store / overlay 表示へ統合）
- [x] HO-207（旧 `CommandBar` は未使用のため削除。操作系は `WorldHudMenu` / `ChatPanel` に統合）
...
- [ ] HO-402
- [ ] HO-403
- [x] HO-404（TopBar「毎日の質問」overlay + 5問マスタ）
- [x] HO-405（`apply-daily-answers` 接続 + 同期率 / Vitals 更新）
- [ ] HO-406
...
- [ ] HO-608（Edge Functions 用 Secret と、Next API routes 用 Vercel server-only env を設定）
- [ ] HO-609
- [ ] HO-610
- [ ] HO-602
- [ ] HO-603
...
| [reference/clone-os-mock.html](./reference/clone-os-mock.html) | UI/3D モック（正） |
...
| 日付 | 内容 |
|------|------|
| 2026-05-25 | 初版 — 放置me プロダクト設計書・参照モックに基づく実装プラン |
...
| 2026-05-25 | **Milestone B**：`houchi-me/` (Next.js 16 + R3F + Zustand + TS) を `frontend/` に昇格し旧 Vite フロントと統合、`houchi-me/` 削除。backend Supabase を houchi-me スキーマ (`profiles/clones/topics/messages/feedback`) に全面置換。AI プロバイダは **Gemini API** に統一（接続抽象は `frontend/src/lib/clone-engine.ts`）。vercel.json/CI env を Next.js 用に更新、root の `node_modules` 削除し `frontend/` で再 install。§0.3 / §1.1 / §2 / §3 / §6 / §10 / §11 を全面改訂 |
