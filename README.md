...
> **1行ピッチ（30字以内）**：放置している間に、エージェントがまだ知らぬ熱狂を見つけ出す
...
| 種別 | URL |
|---|---|
| デモ環境 | https://hochi-me-git-main-k-kankes-projects.vercel.app/ |
| プレゼン資料 | https://www.canva.com/design/DAHKsWlJrzM/q4_bhy0rTS-H7RsH_gLPUQ/edit |
| デモ動画 | （YouTube / Loom） |
...
- **利用 AI ツール**：Google Gemini API（Supabase Edge Functions 経由のクローン Topic 生成・チャット）、Cursor 等（開発）

### 使用した外部 API / サービス

| サービス名 | 用途 | プラン | 備考 |
|---|---|---|---|
| Supabase | DB / Auth / Edge Functions | ハッカソン提供想定 | `NEXT_PUBLIC_SUPABASE_*` は frontend、Gemini 系 secret は Edge Functions 側のみ |
| Google Gemini API | クローンチャット・Topic 生成 | API キー要 | `GEMINI_API_KEY` を Supabase Edge Functions Secret として設定 |
| Vercel | フロント Hosting / Preview | チームプロジェクト | Root Directory = `frontend`（Next.js auto-detect） |

→ frontend の `.env.local` には公開可能な `NEXT_PUBLIC_SUPABASE_*` のみを置き、`GEMINI_API_KEY` などの秘匿情報は Supabase Edge Functions Secret で管理する。
